	...
	C: Configuration + ?Sized,
{
	...
	{
		InnerNodeIter::from_leaf::<C>(self.leaf_index).zip(self.path.iter()).all(
			move |(node, d)| match node.parity() {
	...
	pub fn root(
		...
		self.root_from_base(
			parameters,
			self.leaf_index.join_leaves(parameters, leaf_digest, sibling_digest),
		)
	}
	...
	{
		iter.into_iter().fold(base, Self::fold_fn(parameters, index))
	}
}
	...
	C: Configuration + ?Sized,
{
	...
	where
		InnerDigest<C>: Default,
	{
		Self::fold(parameters, 0, self.leaf_index, base, &Default::default(), &self.path)
	}
		...
		I: IntoIterator<Item = &'i InnerDigest<C>>,
	{
		...
		(depth..path_length::<C, _>()).fold(base, move |acc, _| {
			Self::fold_fn(parameters, index.into_parent(), &acc, default, || iter.next().unwrap())
		})
	}
	...
	C: Configuration + ?Sized,
{
	...
	fn next(&mut self) -> Option<Self::Item> {
		...
		Some((
			node,
			node.parity().right_or_default(|| Some(self.current_path_iter.next().unwrap())),
		))
	}
	...
	C: Configuration + ?Sized,
{
	...
	pub fn from_inner(sibling_digest: LeafDigest<C>, inner_path: InnerPath<C>) -> Self {
		Self { sibling_digest, inner_path }
	}
	...
	pub fn root(&self, parameters: &Parameters<C>, leaf_digest: &LeafDigest<C>) -> Root<C> {
		self.inner_path.root(parameters, leaf_digest, &self.sibling_digest)
	}
	...
	C: Configuration + ?Sized,
{
	...
	where
		...
		InnerDigest<C>: Default,
	{
		self.inner_path.update(parameters, current, &mut self.sibling_digest, next)
	}
}
	...
	InnerDigest<C>: Default + PartialEq,
{
	...
	fn try_from(path: Path<C>) -> Result<Self, Self::Error> {
		let Path { sibling_digest, inner_path } = path;
...
pub mod constraint {
		...
		InnerDigest<C, COM>: ConditionalSwap<COM>,
	{
		...
		pub fn root_from_base(
			...
			Self::fold(parameters, base, self.inner_indices.iter().zip(self.path.iter()), compiler)
		}
		...
		{
			iter.into_iter().fold(base, Self::fold_fn(parameters, compiler))
		}
	}
		...
		fn new_unknown(compiler: &mut COM) -> Self {
			Self {
				...
				inner_indices: (0..path_length::<C, _>())
					...
					.collect(),
				path: (0..path_length::<C, _>()).map(|_| compiler.allocate_unknown()).collect(),
			}
		}
	}
		...
		LeafDigest<C, COM>: ConditionalSwap<COM>,
	{
		...
		pub fn root(
			...
			self.inner_path.root(parameters, leaf_digest, &self.sibling_digest, compiler)
		}
		...
		where
			Root<C, COM>: PartialEq<Root<C, COM>, COM>,
		{
			self.verify_digest(parameters, root, &parameters.digest_with(leaf, compiler), compiler)
		}
	}
