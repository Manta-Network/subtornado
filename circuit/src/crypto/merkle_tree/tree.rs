	...
	C: Configuration + ?Sized,
{
	...
	{
		self.maybe_push_digest(parameters, move || Some(leaf_digest())).unwrap()
	}
	...
	T: Tree<C>,
{
	...
	where
		...
		L: IntoIterator<Item = &'l Leaf<C>>,
	{
		Some(Self::from_tree(T::from_iter(&parameters, leaves)?, parameters))
	}
	...
	where
		Leaf<C>: Sized,
	{
		Some(Self::from_tree(T::from_slice(&parameters, leaves)?, parameters))
	}
	...
	InnerDigest<C>: Clone + PartialEq,
{
	...
	fn prove(&self, item: &Self::Item) -> Option<MembershipProof<Self::Model>> {
		Some(MembershipProof::new(
			self.path(self.position(&self.parameters.digest(item))?).ok()?,
			self.root().clone(),
		))
	}
