	...
	S: SentinelSource<C>,
{
	...
	pub fn with_map_and_sentinel(map: M, sentinel_source: S) -> Self {
		Self { map, sentinel_source, __: PhantomData }
	}
	...
	pub fn map_get_or_sentinel(&self, index: usize) -> &InnerDigest<C> {
		self.map_get(index).unwrap_or_else(move || self.sentinel_source.get(index))
	}
	...
	fn insert_and_get_pair(
		...
		match node.parity() {
			Parity::Left => (self.map_get_or_sentinel(index), self.map_get_or_sentinel(index + 1)),
			Parity::Right => (self.map_get_or_sentinel(index - 1), self.map_get_or_sentinel(index)),
		}
	}
	...
	fn compute_root(
		...
		InnerNodeIter::from_leaf::<C>(leaf_index)
			.fold(base, move |acc, node| self.insert_and_join(parameters, node, acc))
	}
	...
	where
		InnerDigest<C>: Clone,
	{
		InnerPath::new(leaf_index, self.path_iter_for_leaf(leaf_index).cloned().collect())
	}
}
	...
	S: SentinelSource<C>,
{
	...
	fn new(inner_tree: InnerTree<C, M, S>, starting_leaf_index: Node) -> Self {
		Self { inner_tree, starting_leaf_index }
	}
		...
		S: Default,
	{
		...
		let root = node_iter.fold(base, |acc, (node, digest)| {
			...
			match digest {
				Some(digest) =>
					inner_tree.map.set_and_join(parameters, index - 1, digest, index, acc),
				_ => parameters.join(
